...
class LiveKitProvider(TelephonyProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.agent_dispatch_url = config.get("agent_dispatch_url") or (
            f"{BACKEND_API_ENDPOINT.rstrip('/')}/api/v1/telephony/livekit/dispatch"
        )
    ...
    async def _dispatch_agent_join(
        ...
        if not self.agent_dispatch_url:
            raise ValueError("LiveKit agent dispatch URL is not configured.")
